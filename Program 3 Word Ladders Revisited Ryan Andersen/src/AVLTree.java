...
public class AVLTree<AnyType extends Comparable<? super AnyType>>
{
    ...
    private AvlNode<AnyType> remove( AnyType value, AvlNode<AnyType> node )
    {
        ...
        //we need to keep looking for the node
        if( compareResult < 0 )
            node.left = remove( value, node.left );
        else if( compareResult > 0 )
            node.right = remove( value, node.right );

        //we found it, time to remove it
        else if( node.left != null && node.right != null ) // Two children
        {
            //replace the value in node with it's next smaller child, then delete that child
    ...
    private AvlNode<AnyType> balance( AvlNode<AnyType> node )
    {
        ...
        if( height( node.left ) - height( node.right ) > ALLOWED_IMBALANCE )
            if( height( node.left.left ) >= height( node.left.right ) )
                //extra child was on the left, do a right rotation
                node = rightRotation( node );
            else
                //extra child was on the right, do a double rotation
                node = doubleRightRotation( node );
        else
        //heavier on the right
        if( height( node.right ) - height( node.left ) > ALLOWED_IMBALANCE )
            if( height( node.right.right ) >= height( node.right.left ) )
                //extra child was on the right, do a left rotation
                node = leftRotation( node );
            else
                //extra child was on the left, do a double rotation
    ...
    private AvlNode<AnyType> findMin( AvlNode<AnyType> node )
    {
        ...
        //traverse to the leftmost (smallest) node
    ...
    /**
     * Internal method that finds and deletes the smallest node
     * @param node the root of the tree
     * @return root of the new tree after deletion
     */
    private AvlNode<AnyType> deleteMin( AvlNode<AnyType> node )
    {
        ...
        //we found it, time to remove it
            ...
            //make sure not to leave orphaned right children alone
            if(node.right != null){
